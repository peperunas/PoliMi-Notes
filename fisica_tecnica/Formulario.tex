...
\item [{Entropia}] $S=[\frac{J}{\text{°K}}]$, $\bar{s}=[\frac{J}{Kg\cdot\text{°K}}]$;
\item [{Temperatura}] $T=[\text{°}K]$;
\item [{Pressione}] $P=[Pa]$;
\item [{Volume}] $V=[m^{3}]$, $\bar{v}=[\frac{m^{3}}{Kg}]$;
\item [{Massa}] $m=[Kg]$;
\item [{Densità}] $\rho=[\frac{Kg}{m^{3}}]$;
\item [{Velocità}] $w=[\frac{m}{s}]$;
\item [{Costante~Universale~dei~Gas}] $R=[\frac{J}{\text{°K}\cdot kmol}]$,
$R^{*}=\frac{R}{M}=[\frac{J}{Kg\cdot\text{°K}}]$;
\item [{Calore~Specifico}] $c=[\frac{J}{Kg\cdot\text{°K}}]$;
...
\subsection{Generale e Proprietà Gas}
...
\end{tabular}
\begin{itemize}
\item $R=8314,4\frac{J}{\text{°K}\cdot kmol}$;
...
\item $\Delta h=l=v\Delta P$;\footnote{Vale per i liquidi.};
\end{itemize}
...
\item $ds=\frac{dU}{T}$, valido nei liquidi; 
\item $ds=c_{v}\frac{dT}{T}+R^{*}\frac{d\bar{v}}{\bar{v}}$;
\item $ds=c_{p}\frac{dT}{T}-R^{*}\frac{dP}{P}$;
\item $ds=c_{p}\frac{d\bar{v}}{\bar{v}}+c_{v}\frac{dP}{P}$;
\end{itemize}
...
\begin{tabular}{|c|c|}
\hline 
Trasformazione & $n$\tabularnewline
\hline 
\hline 
Isocora $(V=cost)$ & $n\pm\infty$\tabularnewline
\hline 
Isoterma $(T=cost)$ & $n=1$\tabularnewline
\hline 
Isobara $(P=cost)$ & $n=0$\tabularnewline
\hline 
Isoentropica / Adiabatica $(ds=0)$ oppure $(Q=0)$ & $n=\frac{c_{p}}{c_{v}}$\tabularnewline
\hline 
\end{tabular}
\begin{itemize}
\item $P\bar{v}^{n}=cost$;
...
\begin{itemize}
\item $\eta_{isoentropico}=\frac{L}{\overleftarrow{|Q}|}$=$\mbox{\ensuremath{\frac{|\overleftarrow{Q}-\overrightarrow{Q|}}{\overleftarrow{|Q}|}=1-\frac{|\overrightarrow{Q|}}{\overleftarrow{|Q|}}}}$;
...
senza scambio di lavoro né calore per i quali si ipotizzano trascurabili
...
senza scambio di lavoro nè calore per i quali si ipotizzano trascurabili
...
\subsection{Macchine Motrici ($\protect\overrightarrow{L}$)}

Converte energia termica in lavoro. 

\includegraphics[height=0.25\paperheight]{Formulario/MacchinaMotriceComp}
...
dove $Q$ è riferito alla macchina e $T_{C/F}$ è la temperatura associata
al $Q$ corrispondente;
\end{itemize}

\subsection{Macchine Operatrici ($\protect\overleftarrow{L}$)}

Trasferisce energia termica da uno o più serbatoi di calore a temperatura
inferiore a uno o più serbatoi di calore a temperatura superiore.

\includegraphics[height=0.25\paperheight]{Formulario/MacchinaOperatrice}\includegraphics[width=0.5\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/MacchinaOperatriceComp}
\begin{itemize}
\item $\Delta S_{irr}=-\frac{\overleftarrow{Q}}{T_{C/F}}+\frac{\overrightarrow{Q}}{T_{C/F}}$
dove $Q$ è riferito alla macchina e $T_{C/F}$ è la temperatura associata
al $Q$ corrispondente;
...
\begin{enumerate}
\item Le transizioni di fase avvengono con $P=costante$;
\item L'interpolazione per acquisire il titolo $x$ è data, ad esempio a
partire da un volume $v$, da
\end{enumerate}
\[
\mbox{x=\ensuremath{\frac{v-v_{l}}{v_{v}-v_{l}}}}
\]


dove $v_{l}$ è il volume di liquido saturo e $v_{v}$ è il volume
di vapore saturo.


\section{Cicli}

L'indice $n$ nei seguenti cicli è l'indice per una trasformazione
isoentropica. Quindi $n=\frac{c_{p}}{c_{v}}$.
...
Il ciclo di Carnot è costituito da due isoentropiche e due isoterme.

\includegraphics[width=0.5\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/CarnotPV.PNG}\includegraphics[width=0.5\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/CarnotTS.PNG}
\begin{itemize}
...
\includegraphics[width=0.33\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/CarnotVapore}\includegraphics[width=0.33\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/CarnotVaporePV}\includegraphics[width=0.33\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/CarnotVaporeTS}


\subsection{Joule-Brayton}

Il ciclo Joule-Brayton è un ciclo simmetrico ed un sistema aperto
costituito da due isoentropiche e due isobare.

\includegraphics[width=0.5\linewidth,height=0.25\paperheight,keepaspectratio]{Formulario/Brayton_pvTs}
\begin{itemize}
...
\item $\eta_{JB}=1-\frac{T_{1}}{T_{2}}$;
\item $r=\frac{P_{2}}{P_{1}}$ con $r=$ rapporto di compressione;
\item $r_{pmin}=1$;
\item $r_{pmax}=(\frac{T_{3}}{T_{1}})^{\frac{n}{n-1}}$;
\end{itemize}
Affinché si possa operare una rigenerazione bisogna che la temperatura
di fine espansione sia maggiore della temperatura di fine compressione.


\subsection{Diesel}

Il ciclo Diesel è costituito da due isoentropiche, una isocora ed
una isobara.
...
\includegraphics[height=0.25\paperheight]{Formulario/6894054_orig}
...
\includegraphics[height=0.25\paperheight]{Formulario/Rankine_cycle_reheat_TS_sv}
...
\item [{Conducibilità~Termica}] $k=[\frac{W}{m\cdot\text{°K}}]$;
\item [{Coefficiente~Scambio~Termico~Convettivo}] $h=[\frac{W}{m^{2}\cdot\text{°K}}]$;
\item [{Vettore~Flusso~di~Calore}] $\vec{q}=[\frac{W}{m^{2}}]$;
\item [{Viscosità}] $\mu=[Pl]$;
\item [{Diametro~o~Lunghezza~Caratteristica}] $D=[m]$;
\item [{Resistenza}] $R=\frac{\text{°}K\cdot m^{2}}{W}$ oppure $R=\frac{\text{°K}}{W}$;
\end{description}
...
\begin{description}
\item [{Potenza~Generata~su~Unità~di~Volume}] $\sigma=[\frac{W}{m^{3}}]$;
\end{description}
...
\item $Pr=\frac{c_{p}\mu}{k}$, numero di Prandtl;
...
\subsubsection{Proprietà dei numeri adimensionali}
...
\hline 
\multirow{2}{*}{In un condotto circolare} & / & $0.023\cdot Re^{0.8}\cdot Pr^{n}$ & $10^{4}$ & $0,7$ & $160$ & /\tabularnewline
\cline{2-7} 
...
\end{tabular}
\end{itemize}
In un condotto circolare in condizioni di moto turbolento si ha:
...
\item [{Emissività}] $\varepsilon\in[0,1]\subseteq\mathbb{R}$;
\item [{Radiosità}] $J=[\frac{W}{m^{2}}]$;
\item [{Radiazione~Incidente}] $I=[\frac{W}{m^{2}}]$;
\end{description}

\subsubsection{Costanti}
\begin{description}
\item [{Costante~di~Stefan-Boltzmann}] $\sigma=5.67\cdot10^{-8\,}[\frac{W}{m^{2}\cdot\text{°K}}]$;
...
emissività $(0\leq\varepsilon\leq1)$;
\item $E_{n}=\epsilon\sigma T^{4}$, potere emissivo di un corpo alla temperatura
$T$;
\item $(\lambda\cdot T)_{max\,potenza}=2897,8\,[\mu m\cdot\text{°K]}$;
...
la $i$, per cui tutta la radiazione emessa da $i$ è intercettata
da $j$;
\end{itemize}
...
corpi $\Delta T<100\text{°K}$;
\end{itemize}

\subsubsection{Convenzioni per lo scambio termico tra superfici}
\begin{description}
\item [{$q_{1\rightarrow2}$}] Potenza termica per unità di superficie
emessa dalla superficie 1 che incide sulla superficie 2;
\item [{$q_{1-2}$}] Potenza termica per unità di superficie emessa dalla
superficie 1 che viene assorbita dalla superficie 2;
\item [{$q_{1,2}$}] Potenza termica netta per unità di superficie scambiata
...
\end{cases}$
\end{itemize}
Per superfici grigie formanti una cavità:
\begin{itemize}
\item $\dot{Q}_{1,2}=\frac{\sigma_{0}(T_{1}^{4}-T_{2}^{4})}{\frac{1-\epsilon_{1}}{\epsilon_{1}A_{1}}+\frac{1}{A_{1}F_{12}}+\frac{1-\epsilon_{2}}{\epsilon_{2}A_{2}}}$;\end{itemize}
